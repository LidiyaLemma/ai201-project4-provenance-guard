...
1. **Generating the Flask app + Groq signal function (Milestone 3):** Gave the AI tool the Detection Signals section of planning.md plus the architecture diagram and asked it to generate the `POST /submit` route stub and the `groq_signal()` function. The AI tool's first version returned the Groq score embedded inside a larger response dictionary rather than as a standalone float; this was revised so `groq_signal()` returns a plain `float` in [0,1], matching what `compute_confidence()` expected as input, and the function was tested directly with a few sample strings before wiring it into the endpoint.

2. **Validating confidence scoring against real test inputs (Milestone 4):** Used the AI tool to help debug an unexpected result — a clearly AI-generated paragraph ("Artificial intelligence represents a transformative paradigm shift...") was scoring 0.679 ("uncertain") instead of the expected "likely_ai." Asked the AI tool to help inspect the individual signal scores rather than just the combined confidence, which revealed the `llm_score` was high (0.8) but `stylometric_score` was near-neutral (0.498), pulling the weighted average down. Rather than accepting a suggestion to simply lower the `likely_ai` threshold to force the "expected" label, this was overridden in favor of keeping the original 0.75/0.35 thresholds and documenting the stylometric signal's weakness on short text as an honest known limitation instead.
